...
export default function LabDetailPage() {
  ...
  useEffect(() => {
    ...
    const fetchDetail = async () => {
      ...
      try {
        ...
        toast.error(err.message || '获取实验室详情失败');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleStatusUpdate = async () => {
    try {
      await api.put(`/labs/${lab.id}`, { status: selectedStatus });
      const res = await api.get(`/labs/${lab.id}`);
      setLab(res.data);
  ...
  if (!lab) return <div className="p-6">实验室不存在</div>;

  return (
    <div className="container mx-auto p-6">
      <div className="flex items-center justify-between mb-6">
        <Link href="/lab" className="flex items-center border border-gray-300 bg-white hover:bg-gray-300 text-gray-600 py-3 px-3 rounded">
          <span className="mr-1 leading-none">← 返回</span>
        </Link>
        <h1 className="text-2xl font-bold mt-3">{lab.name}</h1>
      ...
      <div className="bg-white shadow rounded-lg border border-gray-200 p-6 my-8 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
        <h2 className="text-lg font-semibold mb-4">实验室信息</h2>
        <div className="border-t border-gray-300/70 my-6"></div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3">
          <div><span className="font-medium">实验室名称：</span>{lab.name}</div>
          <div><span className="font-medium">地址：</span>{lab.address || '未填写'}</div>
          <div><span className="font-medium">容纳人数：</span>{lab.capacity ? `${lab.capacity} 人` : '未填写'}</div>
          <div><span className="font-medium">负责人：</span>{lab.manager_name || '未填写'}</div>
          <div><span className="font-medium">联系电话：</span>{lab.manager_phone || '未填写'}</div>
          <div><span className="font-medium">描述：</span>{lab.description || '无'}</div>
