...
async def create_reservation(
    ...
    """创建预约"""
    try:
        result = await reservation_service.create_reservation(db, reservation_data, current_user.id)
        return BaseResponse(
            code=201,
            message="预约提交成功，等待审批",
            data=result
...
async def list_reservations(
    ...
    result = await reservation_service.get_user_reservations(db, current_user.id, page, page_size, status)
...
async def cancel_reservation(
    ...
    """取消预约（本人或管理员）"""
    try:
        await reservation_service.cancel_reservation(db, reservation_id, current_user.id)
