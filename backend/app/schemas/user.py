...
from app.exceptions.business import UserValidationError
...
class UserCreate(BaseModel):
    ...
    def validate_all(self):
        ...
        if not phone.isdigit() or len(phone) != 11:
            raise UserValidationError("手机号必须为11位数字")

        password = self.password
        if len(password) < 8 or len(password) > 30:
            raise UserValidationError("密码长度必须为8-30位")
        if not re.search(r"[!@#$%^&*(),.?\":{}|<>]", password):
            raise UserValidationError("密码必须包含至少一个符号")
        if not re.search(r"[a-zA-Z]", password):
            raise UserValidationError("密码必须包含至少一个字母")
        if not re.search(r"\d", password):
            raise UserValidationError("密码必须包含至少一个数字")
