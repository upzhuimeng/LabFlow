...
-- Running upgrade  -> b3d166389bb2

CREATE TABLE tag (
    id INTEGER NOT NULL COMMENT '标签ID' AUTO_INCREMENT, 
...
CREATE TABLE lab (
    ...
    tag_id INTEGER NOT NULL COMMENT '标签编号', 
...
CREATE INDEX ix_lab_tag_id ON lab (tag_id);
...
CREATE INDEX ix_tag_log_created_at ON tag_log (created_at);

CREATE INDEX ix_tag_log_operator_id ON tag_log (operator_id);

CREATE INDEX ix_tag_log_tag_id ON tag_log (tag_id);
...
CREATE TABLE tag_user (
    ...
    CONSTRAINT tag_id_user_id_unique UNIQUE (tag_id, user_id)
);

CREATE TABLE tag_user_log (
    id INTEGER NOT NULL COMMENT '日志ID' AUTO_INCREMENT, 
    tag_user_id INTEGER NOT NULL COMMENT '被操作的记录ID', 
    tag_id INTEGER NOT NULL COMMENT '标签ID', 
    user_id INTEGER NOT NULL COMMENT '用户ID', 
    operator_id INTEGER COMMENT '操作人ID', 
    action VARCHAR(20) NOT NULL COMMENT '操作类型(create/update/delete)', 
...
CREATE INDEX ix_tag_user_log_created_at ON tag_user_log (created_at);

CREATE INDEX ix_tag_user_log_operator_id ON tag_user_log (operator_id);

CREATE INDEX ix_tag_user_log_tag_id ON tag_user_log (tag_id);

CREATE INDEX ix_tag_user_log_tag_user_id ON tag_user_log (tag_user_id);

CREATE INDEX ix_tag_user_log_user_id ON tag_user_log (user_id);
...
CREATE TABLE lab_log (
    ...
    name VARCHAR(100) NOT NULL COMMENT '名称', 
    address VARCHAR(255) NOT NULL COMMENT '地址', 
    capacity INTEGER NOT NULL COMMENT '容纳人数', 
    status SMALLINT NOT NULL COMMENT '状态(0-正常 1-维护 2-停用)', 
    tag_id INTEGER NOT NULL COMMENT '标签', 
    keyword VARCHAR(255) NOT NULL COMMENT '关键词', 
    description TEXT NOT NULL COMMENT '实验室说明', 
...
CREATE INDEX ix_lab_log_created_at ON lab_log (created_at);

CREATE INDEX ix_lab_log_lab_id ON lab_log (lab_id);

CREATE INDEX ix_lab_log_operator_id ON lab_log (operator_id);

CREATE INDEX ix_lab_log_tag_id ON lab_log (tag_id);
...
CREATE TABLE reservation (
    ...
    current_level SMALLINT NOT NULL COMMENT '审核阶段(数字为正在审批的层级，0-保存且未提交申请 3-通过所有审批)' DEFAULT '1', 
...
CREATE TABLE approval (
    ...
    level SMALLINT NOT NULL COMMENT '审批等级(1/2)', 
...
INSERT INTO alembic_version (version_num) VALUES ('b3d166389bb2');
